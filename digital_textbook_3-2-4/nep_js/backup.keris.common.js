...
var GUIDE = (function () {
	var guide = {
		start: function () {
			this.mainStart();
			if (document.querySelectorAll('.guideBtn').length > 0) this.sourcePopupStart();
		},
		mainStart: function () {
			var goMainBtn = document.getElementById('goMain');
			if (goMainBtn) goMainBtn.addEventListener('click', this.goMain);
		},
		sourcePopupStart: function () {
			this.popup = document.getElementsByClassName('popup');
			this.popupArea = document.querySelector('#popupArea');
			this.guideBtnInPopup = popupArea.querySelectorAll('.guideBtn');
			this.popupContainer = document.getElementById('popupContainer');

			var guideBtn = document.getElementsByClassName('guideBtn');
			if (guideBtn) {
				for (var i = 0; i < guideBtn.length; i++) guideBtn[i].addEventListener('click', this.sourcePopupOpen.bind(this, guideBtn[i].getAttribute('about')));
			}

			var sourcePopupCloseBtn = this.popupContainer.querySelector('.popupCloseBtn');
			if (sourcePopupCloseBtn) sourcePopupCloseBtn.addEventListener('click', this.sourcePopupClose.bind(this));
		},
		goMain: function () {
			window.location.replace('./main.html');
		},
		sourcePopupOpen: function (type) {
			GUIDE.popupHide();

			for (var i = 0; i < this.guideBtnInPopup.length; i++) {
				if (this.guideBtnInPopup[i].getAttribute('about') === type) {
					this.popup[i].style.display = 'block';
					this.guideBtnInPopup[i].classList.add('active');
				}
			}
			this.popupContainer.style.display = 'block';
		},
		sourcePopupClose: function () { console.log('sourcePopupClose')
			document.getElementById('popupContainer').style.display = 'none';
			GUIDE.popupHide();
		},
		popupHide: function () {
			for (var i = 0; i < this.popup.length; i++) {
				this.popup[i].style.display = 'none';
				if (this.guideBtnInPopup[i].classList.contains('active')) this.guideBtnInPopup[i].classList.remove('active');
			}
		}
	}
	guide.start();
