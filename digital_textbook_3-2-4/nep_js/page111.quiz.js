...
var QUIZ = (function () {
	var quiz = {
		...
		correctCheck: function (curQuiz) {
            ...
            if (curQuiz.sq == 8)
            {
                curQuiz.isCorrect = showDropAnswer(false);
                var userAnswer = "";
                $(".dragDrop").each(function(){
                    userAnswer += (!$(this).is(':empty'))? $(this).find("div").attr("id").split("_")[2] : "X";
                });
                curQuiz.userValue = userAnswer;
            }
            if (curQuiz.sq == 10)
            {
                var isCorrect = true;
                var userAnswer = "";
                $(".quizOX").each(function(){
                    ...
                    if ($(this).hasClass('quizOXSelected')) userAnswer += $(this).attr("data-ox");
                });
                curQuiz.isCorrect = isCorrect;
                curQuiz.userValue = userAnswer;
            }

			// ------------------------------------------------------------------ //
            console.log('index: ', curQuiz.sq);
			...
			if (curQuiz.isCorrect) $("assessmentItem[quiz-index='"+curQuiz.sq+"'] > .answer_img > .answer_o").show();
			else
			{
				$("assessmentItem[quiz-index='"+curQuiz.sq+"'] > .answer_img > .answer_x").show();

				// .complete .singleChoiceObj[answer]:not(.selected):after {background:url(...)} 안보이는 문제로 강제로 가리기/보이기
				$("assessmentItem[quiz-index='"+curQuiz.sq+"'][class='complete'] > itemBody").hide().delay(1).queue(function (next) {
					$(this).show();
					next();
				});
			}
...
function Quiz (container) {
	...
	this.sq = container.getAttribute('quiz-index');
...
function FILLINTHEBLANK (quiz) {console.log('FILLINTHEBLANK Start---------->');
	...
	this.watchProperty = function (self) {
		var watch = setInterval(function () {
			for (var i = 0; i < self.inputContainer.length; i++) {
				if (self.inputContainer[i].querySelector('input'))
				{
				...
				else if (self.inputContainer[i].querySelector('textarea'))
				{
					var input = self.inputContainer[i].querySelector('textarea');
					if (input.value !== '') {
						self.keyDown(self.inputContainer[i]);
						clearInterval(watch);
					}
	...
	this.keyDown = function (obj, index) {
		...
		{
			console.log("input length :"+obj.querySelectorAll('input').length);
			if (obj.querySelectorAll('input').length > 1)
			{
				for (var i = 0; i < obj.querySelectorAll('input').length; i++) {
					if (i > 0) curUserValue += "|";
					curUserValue += obj.querySelectorAll('input')[i].value;
				}
			}
			else
				curUserValue = obj.querySelector('input').value;
		}
		else if (obj.querySelector('textarea'))
			curUserValue = obj.querySelector('textarea').value;
		
		console.log(curUserValue);
	...
	this.clearInputValue = function () {
		[].forEach.call(this.inputContainer, function(val, idx){
			...
			{
				if (val.querySelectorAll('input').length > 1)
				{
					var input;
					for (var i = 0; i < val.querySelectorAll('input').length; i++) {
						input = val.querySelectorAll('input')[i];
						input.value = '';
					}
				}
				else
			...
			else if (val.querySelector('textarea'))
				var input = val.querySelector('textarea');
			input.value = '';
...
function DRAWLINE (quiz) {console.log('DRAWLINE Start---------->');
	...
	this.answerToggle = function () {
		if (this.answerMode == 'answer') this.answerMode = 'reset';
		else {
			this.answerMode = 'answer';
			quiz.container.classList.remove('complete');
		}
	...
	loadScriptFile('./nep_js/keris2.drawLine.js', function(){
		var interval = setInterval(function(){//log(ZOOM.rate);
			if (ZOOM.rate) {
...
function ETC (quiz) {console.log('ETC Start---------->');
	this.reset = function () {
        if (quiz.sq == 8) showDropDefault();
        else if (quiz.sq == 10)
        {
            $(".quizOX").each(function(){
